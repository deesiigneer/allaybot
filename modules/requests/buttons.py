...
class ButtonRecruiting(View):
    ...
    def __init__(self, guild: Guild = None):
        ...
        sql_recruiting = sql.get_requests(guild_id=guild.id) if guild is not None else None
...
class ApplicationToCityButtons(View):
    ...
    def __init__(self, interaction: Interaction = None):
        ...
        self.sql_recruiting = sql.get_requests(interaction.guild.id) if interaction is not None else None
        super().__init__(timeout=None)
    ...
    async def recruiting_edit_resume(self, button: Button, interaction: Interaction):
        from handler import update_resume_preview, check_user_pass
        check_pass = await check_user_pass(interaction.user)
    ...
    async def recruiting_status(self, button: Button, interaction: Interaction):
        if self.sql_recruiting is None:
            self.sql_recruiting = sql.get_requests(interaction.guild.id)
        ...
        message = interaction.guild.get_channel(self.sql_recruiting['requests_channel_id']).get_partial_message(self.sql_recruiting['requests_message_id'])
...
class RecruitingModal(Modal):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if self.preview is True:
            ...
            channel = interaction.guild.get_channel(sql_recruiting['requests_channel_id'])
            from handler import update_resume_preview
            preview_labels = []
            for label in self.labels:
                preview_labels.append(label.value)
            ...
            if update_resume_preview == [None, None]:
                ...
                citizen = interaction.guild.get_role(sql.get_guild(interaction.guild.id)['citizen_role_id'])
                message = await thread.send(content=f'{citizen.mention}, проголосуйте за принятие нового жителя!',
                                            embed=update_resume_preview[0])
